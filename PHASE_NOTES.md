...
**What's NOT verified**: the restore drill's correctness against a real Postgres instance. This dev environment has no local Postgres reachable this session (confirmed directly — `npx prisma db execute` hung against `localhost:5432` with nothing listening, no Docker available to start the project's own `docker-compose` Postgres either) — every restore-drill assertion above is against a mocked Prisma client, proving the *logic* (order, remapping, cleanup) is correct, not that real RLS policies and real FK constraints accept what this code sends them. The 46,750-byte real backup from the prior phase's own live run is a real file this drill's logic is designed to read — running `runRestoreDrill` against that actual file, against a real running Postgres, is the one honest gap here, worth doing before this ships to anything real rather than trusting the mocked proof alone.
