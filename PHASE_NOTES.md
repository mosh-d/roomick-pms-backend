...
### This is the first part of the app reachable without a JWT, and it's designed around that
Everything before this sat behind the global `JwtAuthGuard` → `TenantGuard` → `RolesGuard` chain, with only `auth` and `system/health` marked `@Public()`. A guest booking a room has no account, so this module is the first genuinely anonymous write path in the system. The constraints that follow from that drove most of its design:

- **The public DTO is deliberately NOT a reuse of `CreateReservationDto`.** That DTO is a staff input surface and accepts four things the public must never set: `guestId` (attach a booking to someone else's guest profile by id), `channel` (make a direct booking masquerade as an OTA one, corrupting channel-mix reporting and, once the channel manager lands, commission reconciliation), `corporateAccountId` (claim a negotiated corporate rate by guessing an id), and `joinWaitlist` (skip the availability check entirely and create rows against a full house). `PublicCreateReservationDto` accepts none of them and the service forces all four server-side. Verified live: each one now returns 400 `property <x> should not exist`, because `forbidNonWhitelisted: true` rejects them outright rather than silently ignoring them.
- **Every failure mode of slug resolution returns the same generic 404.** An unpublished-but-real property must not be distinguishable from one that never existed, or the endpoint becomes a property-enumeration oracle.
- **Tighter per-IP throttling than the app-wide 100/min**: 60/min on reads, and 10/hour on the single public write — generous for a real guest (who books once) and hostile to a bot filling a property's inventory with junk.
- **The publish/unpublish controls live on a separate controller class** specifically so the class-level `@Public()` can't leak onto them. Verified live: `PUT /branches/:id/booking-engine` with no token returns 401.

### `booking_slug_index` exists because `branches` has FORCE RLS — the same problem login already solved
A public booking request arrives with no tenant context at all, so it can't set `app.tenant_id`, so a lookup against `branches` returns **zero rows, always**. This is exactly the problem login had ("which tenant owns this email?") and it's solved the same way: a small, deliberately non-RLS-scoped pointer table, mirroring `user_email_index` down to the reasoning in its schema comment. It holds nothing but slug → (tenantId, branchId).

The pointer is never trusted on its own. `PublicBookingService.resolveBookableBranch` re-loads the real branch inside `withTenant` and re-checks `bookingEngineEnabled`/`deletedAt`, then checks the owning tenant's status, before anything is served. A suspended or cancelled tenant's properties stop taking bookings; a trialing tenant's don't (a trial property is genuinely open for business).

### `Tenant.subdomain` was NOT reused as the public identity, despite what the timeline doc assumed
The growth plan proposed `book.[subdomain].roomick.app`. The schema disagrees, in writing: `Tenant.subdomain` is commented as "Backend-internal identifier only… never user-typed or shown… nothing else in the app consumes it as a real subdomain/URL", it can carry a random hex collision suffix, and it identifies a **tenant group**, not a bookable property — a group with five hotels needs five booking pages, not one. So the public identity is a new per-branch `bookingSlug` instead. Both new Branch columns are opt-in (NULL slug, engine disabled by default), so every existing branch is unchanged and accepts no public bookings until its owner publishes it. Unpublishing keeps the slug reserved to that branch rather than releasing it for someone else to claim.

### Three small widenings to shared code, no forked logic
A guest booking is an **ordinary reservation through the ordinary path** — the same `createReservation` a front-desk agent's booking goes through, so the capacity check, the Revenue Management restrictions, the `FOR UPDATE` room-type lock that closes the double-book race, the availability check, the Rate Resolver cascade, confirmation numbering, the audit row and the confirmation comms-log entry all apply unchanged. Making that possible needed only:
1. `createReservation`'s `actorId` widened to `string | null`, and the private `audit` helper with it. **No migration** — `Reservation.createdBy` was declared nullable at P0 with the comment "NULL = online booking", and `AuditLog.userId` with "NULL = system action". The schema anticipated this feature three phases early.
2. `resolveStay` gained `persistAudit` (defaulting true, so every existing caller is untouched). See below.
3. A new `getAvailabilityForRange` on `ReservationsService` — all room types across an arbitrary date range, which is the shape a booking widget needs and neither existing method had (`getAvailability` is one room type, `getAvailabilityCalendar` is locked to a calendar month). It loops the same `computeAvailabilityPerNight` inside one transaction, for the reason `getAvailabilityCalendar`'s own comment already gives.

The strongest evidence this really is the shared path: a Revenue Management minLOS restriction, configured through the internal API, correctly rejects a **public** booking with 409 `RESERVATION_NOT_AVAILABLE` — verified live. Nothing in this module knows restrictions exist.

### A real problem caught while building: anonymous quotes were about to write unbounded audit rows
`resolveStay` writes one `rate_audit_log` row **per night, per call**. That's fine for staff-triggered quotes (low volume, authenticated) but a booking widget re-quotes on every date and room-type change a browsing visitor makes, and this endpoint is anonymous — so a scraper, or just ordinary browsing traffic, would balloon that table with rows nobody can act on. The trail exists to defend a disputed charge; **a quote nobody booked has no charge to dispute**. So the public quote path passes `persistAudit: false`, and the moment that same guest actually books, the ordinary create path resolves again and does persist and link the trail properly. Re-implementing pricing to avoid the writes was never on the table — that would have broken the "rate parity is structural, not a promise" property this whole feature rests on.

### Payment is honestly out of scope
The confirmation says, in visible copy, that payment is taken at the property on arrival. That's the truthful statement today: tokenized card payments are Month 11, and no payment-processor integration exists anywhere in this app. The timeline's own Month 7 notes anticipated this ("a 'pay at property' fallback ships first so this month doesn't block on that one").

### Channel Manager, and why it isn't here
The other half of Month 7 needs a real SiteMinder-class aggregator account and credentials. The mapping table, ARI delta-push job and rate-parity monitor could all be written against the spec, but not one real OTA reservation could be verified end-to-end — it would ship as scaffolding that looks finished and isn't, the same category as the Payment Gateway card and the unwired webhook delivery. Deferred until there's an actual aggregator account to integrate against. Worth noting the schema is already further along than the timeline assumed: `ReservationChannel` has included `booking_com`/`expedia`/`agoda`/`airbnb` since P0, so no enum work remains.

### Verified
`npx tsc --noEmit`, `npm run lint` (0 errors), `npm test` — 572 tests, all green (23 new, concentrated on the security boundaries: every slug-resolution failure mode returning an identical generic 404; the index never being trusted without re-checking the real branch; suspended/cancelled tenants refused and trial tenants served; `getProperty` returning an exact allow-list of public-safe keys; quotes never persisting audit rows; and each of the four forced fields on the create path).

While running the suite, **5 pre-existing `walkIn` tests were found failing for a reason unrelated to this work**: their DTO hardcoded `checkOutDate: '2026-09-04'`, and `walkIn` deliberately forces `checkInDate` to *today*, so the fixture silently became a past date once real time passed it — a time bomb that would have started failing on Sept 4 regardless. Fixed by computing the date forward from today, with a comment saying why a literal can't be used there.

Live, against real Postgres (38/38 checks): a branch is unpublished by default and its slug 404s; publishing makes every public endpoint resolve with **zero auth headers**; the property payload carries no tenantId/branchId/policies; room types return real amenities, bed type and rates; availability reports the real pool and decrements after a booking; a quote returns a real Rate Resolver result; a guest self-books and the result is a real reservation visible to staff with `channel: 'direct'` and `createdBy: null`; all four field-spoofing attempts are rejected; a past-dated booking is rejected; a Revenue Management restriction blocks a public booking and then permits a compliant one; a second tenant's slug resolves only to its own property and tenant A's `roomTypeId` is unreachable through it; a taken slug returns 409; unpublishing kills the URL immediately while keeping the slug reserved; and the publish route itself still requires auth. See the frontend's own `PHASE_NOTES.md` for the guest-facing UI pass.

### Carried forward
Channel Manager (needs a real aggregator account) · card payment at booking (Month 11) · ~~a Property Config UI for the publish/slug controls~~ — **shipped same day**, see the frontend's Phase 65 · emailing the guest their confirmation (the comms log records it; no real sending provider is wired) · multi-property landing page for a group with several published branches.

## Revenue Management — the TRUE last of the 11 architecture-map gaps, and a real bug the live pass caught (2026-08-30)

The one item Sales & Events (below) deliberately deferred, because two of its four reference cards are labeled "AI-based"/"AI-suggested" and this app has no ML/forecasting infrastructure anywhere. Resolved by building real, honestly-labeled statistical/rule-based features instead of either fabricating an AI claim or leaving the gap open — the same choice already made for Comp Set Analysis (stays inert; a real one needs an external competitor-rate feed, out of scope here).

### Restrictions Management — a real booking-blocking mechanism, deliberately distinct from `RatePlan.minLOS`
New `AvailabilityRestriction` model (branch-scoped, optional `roomTypeId` — null means "every room type at this branch"): `minLOS`/`maxLOS`/`closedToArrival`/`stopSell` over a date range. `RestrictionsService.assertNoViolation` is called from inside `ReservationsService.createReservation`'s existing transaction, right after `assertWithinCapacity`, on the resolved room type and dates — additive only, so a tenant that never configures a restriction sees zero behavior change (the query simply returns nothing to violate). Deliberately NOT wired into `walkIn`: a same-day walk-in is exempt from length-of-stay restrictions in practice, and touching a second already-tested method wasn't judged worth folding into this pass. This is a genuinely different concept from the pre-existing `RatePlan.minLOS` — confirmed by reading `RateResolverService` before writing a line of new code — which only decides which rate-cascade tier is eligible for a stay length; it has never blocked a booking outright. Wiring the new dependency into `ReservationsService`'s constructor broke that service's own spec file (117 failures, all NestJS DI-resolution errors from the unmocked new provider) until a proper mock was registered — a real, necessary fix to the test suite's DI graph, not a workaround.

### Demand Forecast and Rate Recommendations — real, honestly non-AI, and never claim to be
`DemandForecastService.getForecast` reuses `ReportsService.getOccupancy`'s already-correct room-night math: a same-weekday historical average over the trailing 8 weeks, projected forward. `forecastOccupancyPct: null` means "not enough historical data for this weekday yet" — never a fabricated number. `RateRecommendationsService` layers fixed thresholds on top (≥75% occupancy → suggest +15%, ≤25% → suggest −10%, otherwise no change) and `approveRecommendation` creates a real `seasonal` `RatePlan` for that single date through the SAME `RateResolverService.createRatePlan` every other rate plan in this app already goes through — confirmed via reading `RateResolverService`'s cascade math (`running = before.add(before.mul(plan.amount).div(100))`) that a signed negative `amount` produces a genuine discount through the identical code path a positive one uses to raise the rate, so no `Math.abs()` or parallel mechanism was needed. The frontend page states outright, in visible copy, "not AI" next to the Rate Recommendations section — not just absence of the word, an active disclaimer.

### A real bug the live-verification pass caught: pre-existence days were silently counted as genuine 0% occupancy history
`Room` has no "existed as of this date" concept — `ReportsService.getOccupancy` uses TODAY's room inventory for every date it's asked about, including ones before the branch existed, and always returns a real trend entry (never omitted) with `occupancyPct: 0` when nothing was sold. Left unclamped, a demand forecast for a brand-new branch would silently treat every one of the 8 trailing weeks as genuine "0% occupancy" history — a real-looking number that isn't real data, quietly dragging every future recommendation toward "occupancy is low, discount the rate" for a hotel that hasn't even opened yet. Caught live: the very first verification run showed a fabricated 0%/historicalSampleSize-8 forecast for a branch created moments earlier in the same test, which then triggered real (wrong) discount recommendations. Fixed in `DemandForecastService.getForecast` by clamping the lookback window's start to the branch's own `createdAt`, and treating "today" as the query's exclusive upper bound (a day still in progress isn't a completed historical data point either) — a branch created today now correctly reports `null` for every weekday, honoring the contract this module's own header comment already promised but didn't actually keep.

### Verified
`npx tsc --noEmit`, `npm run lint` (0 errors), `npm test` — 549 tests, all green (23 new: restriction CRUD; `assertNoViolation` tested against stop-sell, closed-to-arrival with an in-window/out-of-window boundary check, minLOS/maxLOS with exact-boundary cases, and the branch-wide-vs-room-type-scoped `OR` query shape; demand forecast's weighted-average math with real date arithmetic, the branch-creation clamp, and the single-day collapse for a same-day-created branch; rate recommendation thresholds for high/low/typical occupancy and the "not enough data" case; approve rejecting a zero adjustment and passing a negative adjustment through unmodified). `reservations.service.spec.ts` gained 2 tests confirming the new restriction check is called with the resolved branch/room type/dates and that a violation propagates without ever reaching `tx.reservation.create`.

Live, against real Postgres: created minLOS/maxLOS/closed-to-arrival/stop-sell restrictions (including a branch-wide one with no `roomTypeId`) and confirmed each correctly rejects a violating booking (409 `RESERVATION_NOT_AVAILABLE`) while an otherwise-identical non-violating booking succeeds; confirmed a fresh branch with zero restrictions sees zero behavior change; confirmed deleting a restriction actually lifts the block on a subsequent booking attempt. Confirmed (and initially caught failing, then fixed) the demand-forecast bug above. Approved both a +15% and a −10% recommendation and confirmed each created a real, single-day `seasonal` RatePlan with the correct signed percentage, visible among the branch's own rate plans exactly like a manually-created one. Frontend: all 4 sections render, the page never claims to be AI-based/AI-suggested, restriction create/remove round-trips through the real UI, and the Rate Recommendations table correctly shows zero Approve buttons (an honest em-dash instead) when there's genuinely no signal yet, rather than a fabricated suggestion.

### Closing note
This closes the true final item of the original 11 architecture-map gaps. Every one is now either fully built or has an honestly-scoped partial build with explicitly-stated, deliberate limitations — none was silently skipped, half-built without saying so, or dressed up with a capability (AI included) that doesn't actually exist.
...
### New models, `ApiKey` and `Webhook`, RLS hand-written (not automatic for a new table)
Migration `20260830030000_integrations_api_keys_webhooks` — same shadow-database workaround (`migrate diff` + `migrate deploy`) and the same hand-written `ENABLE`/`FORCE ROW LEVEL SECURITY` + `tenant_isolation` policy this session has now used three times for a new tenant-scoped table (`GuestNote`, then these two).

### API Keys — SHA-256, not bcrypt, and a deliberate choice not to build a second auth path
The raw key is generated, returned to the caller exactly once, and only its SHA-256 hash is ever persisted — bcrypt (this codebase's own password-hashing choice) is the wrong tool here: it's a slow KDF meant to resist brute-forcing a LOW-entropy human-chosen password, and buys nothing against a 48-hex-char high-entropy random token that's already effectively unguessable. `verifyApiKey` exists and is tested, but nothing in this codebase actually calls it — no route accepts an API key as an alternative to the existing JWT. Building a second, self-rolled authentication mechanism to "prove" the feature end-to-end would have been a materially riskier, separate piece of scope than CRUD over a new table; stated honestly instead, the same class of restraint as GDPR erasure staying human-triggered and the Permission Matrix's permissions staying unenforced.

### Webhooks — subscription CRUD, delivery deliberately not wired up
Storing a URL + event types + a signing secret is real and complete on its own. Actually TRIGGERING a webhook on a real PMS event would need a single choke point every mutating action passes through — checked directly and confirmed this app doesn't cleanly have one: `AuditInterceptor` fires generically for every mutating request, but most services ALSO hand-roll their own private `audit()` method with richer, more specific action names, so no single centralized dispatch point already exists to hook into safely. Wiring delivery through the generic interceptor alone would work but only for its own coarse `entity.method`-style events, and modifying a piece of infrastructure every request in the app flows through for what's fundamentally a nice-to-have carries more risk than this pass's own scope justified. "Delete" is a soft `isActive: false`, never a hard delete — matching `FeatureFlag`'s own schema comment ("never delete").

### A real bug caught by a verification script's own deliberately-malformed input, not by review
`CreateWebhookDto`'s first draft used `@IsUrl({ require_tld: false })` to allow local/dev partner endpoints (`http://localhost:4000/hook`) during integration testing. `validator.js`'s own `require_protocol` defaults to `false`; combined with `require_tld: false`, a bare word like `"not-a-url"` validated as a legitimate "host-only" URL and the endpoint returned 201, not a rejection — caught live when a verification script's own deliberately-invalid-URL check failed. Fixed by adding `require_protocol: true` explicitly, which still allows `http://localhost:4000/hook` (protocol present, no TLD required) while correctly rejecting anything with no scheme at all.

### Verified
`npx tsc --noEmit`, `npm run lint` (0 errors), `npm test` — 507 tests, all green (12 new: raw key returned once and never re-derivable from the stored hash; list/summary shapes never include `keyHash`/`secret`; revoke/deactivate 404 on an unknown id; `verifyApiKey` correctly resolves a real non-revoked key and rejects an unknown one while stamping `lastUsedAt` only on success; deactivation is soft, never a delete).

Live, against real Postgres: created and revoked a real API key via the full HTTP surface, confirmed the list response never contains the raw key or its hash at any point; created and deactivated a real webhook, confirmed it stays in the list after deactivation (never disappears) with the secret never re-exposed; confirmed a genuinely malformed URL is rejected with 400 while a localhost dev URL is accepted. See the frontend's own `PHASE_NOTES.md` for the UI pass, including the exact same URL-validation bug being what the live suite caught first.

### Carried forward
Payment Gateway (the reference's third Integrations & APIs card) is NOT represented anywhere in this pass — no real payment-processor integration exists in this app to configure, and a form storing gateway "credentials" nothing ever reads would be actively misleading, not an honest partial feature. 2 of the original 11 architecture-map gaps remain: Revenue Management, Sales & Events — both still needing real new domain design.
...
### The real fork: there is no cross-tenant "SysAdmin" concept anywhere in this app
The reference's own badge for this section is "SysAdmin," implying a platform-wide console — flags/backups/health across ALL tenants. But every role (`SystemRole`) is per-tenant, `TenantGuard` requires a matching `X-Tenant-ID` header against the JWT on every non-`@Public()` route, and nothing bypasses that anywhere except `@Public()` itself. Building genuine cross-tenant admin auth would mean a real, security-sensitive change to the core auth model — not something to improvise silently under a feature-page task. Surfaced directly and asked: build tenant-scoped only (an Owner's own self-service view — their own tenant's backups, their own tenant's flag membership, a shared read of the one running process), which is what got built. Real cross-tenant platform admin stays a named, deferred architectural decision, not something quietly half-built.

### Feature Flags — self-service opt-in/out, never global control
`FeatureFlagsService.listFlags` resolves `enabledForThisTenant = enabledGlobally || enabledForTenants.includes(tenantId)` server-side and **never returns the raw `enabledForTenants` array** — it holds every opted-in tenant's UUID, and handing that back to any Owner would leak which OTHER tenants have a flag on, a real cross-tenant data leak the shape avoids by resolving down to one boolean per flag. `setEnabledForTenant` only ever adds/removes the CALLING tenant's own id — never `enabledGlobally`, `rolloutPct`, or another tenant's membership; creating new flags stays genuine platform/engineering territory (a flag corresponds to an actual code branch) with no console built for it. Seeded exactly one real example via a data-only migration (`20260830020000_seed_loyalty_feature_flag`) — `loyalty_module`, matching the schema's own comment example, tied to the still-unbuilt Loyalty & Marketing item in this same sequence. Stated honestly in the UI: no flag gates any real behavior yet, the same "stored/resolvable but not yet enforced" caveat already established for the Permission Matrix's own unenforced permissions.

### Backups — closing a real security hole while adding the missing controller
`BackupRecord` has **no RLS** (a global system table, `tenantId` nullable for full-system dumps) — building the controller naively (just forwarding `backupId` to `verifyBackup`/`runRestoreDrill`) would have let any tenant's Owner verify or restore-drill ANOTHER tenant's backup just by guessing a UUID. Added `assertOwnedBackup` (a `findFirst({ id, tenantId })` check) in front of both operations via new `verifyOwnedBackup`/`restoreDrillOwnedBackup` wrappers — the cron-triggered all-tenants sweeps still call the underlying methods directly, since they already iterate per-tenant with a known-correct id. `listBackups` stringifies `sizeBytes` (a `BigInt` column) — the same JSON-serialization crash `runTenantBackup`'s own comment already fixed for the snapshot file applies equally to a plain API response.

### System Health — the honest subset of "CPU, DB, API response times, error rates" actually measurable
No `@nestjs/terminus`, no Prometheus client, no request-timing infrastructure existed anywhere. Added a genuinely new `MetricsService` (`src/common/metrics/`, global via `CommonModule`) holding a rolling 15-minute in-memory window of `{durationMs, isError}` samples, fed by a new `MetricsInterceptor` registered FIRST among the `APP_INTERCEPTOR`s (so its own `next.handle()` times the full remaining pipeline) — covering every request including `@Public()` ones, for an honest picture of real traffic. CPU is deliberately NOT reported: `process.cpuUsage()` only means anything as a delta over a measured interval, and there's no `os.loadavg()` equivalent on Windows — reporting a fake or misleading CPU number would be worse than omitting it. `GET /system/health/detailed` (Owner-only, new) extends the existing DB check with measured latency, process uptime, and memory, alongside the metrics snapshot; the original `@Public() GET /system/health` is untouched.

### Verified
`npx tsc --noEmit`, `npm run lint` (0 errors), `npm test` — 483 tests, all green (19 new: `MetricsService`'s rolling-window pruning and average/error-rate math; `FeatureFlagsService`'s resolution logic, the never-leak-other-tenants'-ids guarantee, add/remove idempotency, 404 on an unknown flag; `BackupsService`'s new `listBackups` BigInt-to-string mapping and the ownership-check rejecting/delegating correctly; `SystemService.detailedHealth`'s shape under both up and down DB states).

Live, against real Postgres, with TWO real tenants specifically to prove isolation: registered tenant A and tenant B, confirmed toggling the flag on for A left B's own resolved status completely unaffected; confirmed tenant B attempting to verify or restore-drill tenant A's real backup both correctly 404 (not silently succeeding or leaking existence); triggered a real backup for tenant A, verified it (`ok:true`, all 38 tables intact), then ran a full restore-drill (`ok:true`, every model's expected/restored count matched) — the complete backup lifecycle exercised end-to-end through the new HTTP surface, not just the pre-existing service in isolation; confirmed the detailed health endpoint rejects an unauthenticated caller and reports real, non-zero request-metrics counts reflecting the actual API traffic from this very test run. See the frontend's own `PHASE_NOTES.md` for the UI pass.

### Carried forward
Real cross-tenant platform admin (a genuine "SysAdmin" role/allowlist bypassing `TenantGuard`) remains an explicit, deferred architectural decision — not built, and not silently worked around. 5 of the 11 gaps remain: Loyalty & Marketing (display-only slice), Revenue Management, Sales & Events, Integrations & APIs, Enterprise/HQ.
...
### Audit Log Viewer — the fully new, low-risk piece
`src/modules/audit-logs/` (new) — `AuditLogsService.listAuditLogs`, one paginated `GET /audit-logs?branchId=&userId=&action=&from=&to=&page=&limit=` query over the `AuditLog` table every other module already writes to. `id` is a `BigInt` (autoincrement) — stringified the same way `RateResolverService.getAuditTrail` already does, since Nest's JSON serializer throws on a raw BigInt otherwise. `action` matches as a case-insensitive substring (not exact) so filtering by `gdpr` surfaces every `gdpr.*` action at once; a `to` date is treated as inclusive of the whole calendar day. `@Roles(Owner, Manager)` — read-only, no write surface at all.

### GDPR Compliance — real Prisma models that had sat fully unused since P0
`GdprRequest`/`GdprType`/`GdprStatus` already existed in the schema (checked directly via grep before writing anything — zero references anywhere in `src/`) with `deadline`, `exportUrl`, and `notes` columns already modeled, exactly the same "schema exists, nothing built on it yet" pattern already found for `MaintenanceOrder`/`Asset` and `GuestProfile`'s CRM fields. `src/modules/gdpr/` (new), `@Roles(Owner)` only:
- **`createDataRequest`** — `deadline = requestedAt + 30 days` (the GDPR Article 12(3) statutory response window, the figure the schema's own comment already names). `verificationMethod` has no dedicated column (the reference names it as a UI field, but the model has none) — folded into `notes` at creation rather than silently dropped.
- **`downloadExport`** — lazy: generates the export on first call, serves the same stored file on every later call, matching the reference's own single `GET .../export` endpoint rather than a separate generate step. Builds a real JSON payload (guest profile via the existing `GuestsService.getGuestDetail(..., reveal: true)`, reservations, folios with line items/payments, comms log), encrypts it (`EncryptionService.encryptBuffer`) and stores it through the same `DocumentStorageAdapter` registration cards/ID photos already use — no new storage mechanism invented. Rejects `erasure`-type requests outright (`BadRequestException`) — there is nothing to export for one.
- **A deliberate, named safety boundary**: this module performs NO automated cross-table PII deletion anywhere. Safely erasing a guest means touching reservations, folio line items/payments, comms history, and audit rows — several of which carry independent financial/legal retention requirements this pass has not worked out, and inventing that logic casually would be genuinely dangerous, not a quick add. An `erasure` request is created and tracked through status like any other, on the honest assumption that the actual erasure work happens through a real ops process outside this system. `PATCH /gdpr/data-requests/:id/status` (new, beyond the reference's literal endpoint list) lets an Owner manually progress a request to `in_progress`/`completed`/`rejected` with notes — without it, an erasure request would sit at `pending` forever with no way to reflect that the real-world work actually happened. `completed`/`rejected` are terminal; a second status change attempt is rejected (409).

### Verified
`npx tsc --noEmit`, `npm run lint` (0 errors), `npm test` — 422 tests, all green (8 new for `AuditLogsService`: pagination defaults, each filter individually, the from/to inclusive-day range, BigInt-to-string; 15 new for `GdprService`: guest-not-found, the 30-day deadline math, verificationMethod folding into notes, terminal-status rejection, completedAt only set on `completed`, existing notes preserved when none given, erasure export rejection, lazy generate-once-then-reuse for the export file).

Live, against real Postgres: fetched the 6 seeded roles, PUT a permission map onto `manager` and confirmed it round-tripped exactly; created a guest and filed both an `access` and an `erasure` request; confirmed exporting the erasure request is rejected (400) while the access request's export downloads real guest data (matching the guest's actual name); confirmed the request auto-completed once exported; manually progressed the erasure request to `completed` via the status endpoint and confirmed a further status change is rejected (409, terminal); confirmed the audit log picked up all three new action types (`gdpr.request_created`, `gdpr.data_exported`, `gdpr.status_changed`) via the new `/audit-logs` endpoint. See the frontend's own `PHASE_NOTES.md` for the full UI walkthrough.

### Carried forward
- "Custom role creator" and "preset role templates" (both named in the architecture map) are NOT built — there is no `POST /auth/roles` endpoint to create a new role against, and none was added; editing an EXISTING role's permissions is real.
- 9 of the 11 Management/Admin gaps remain — Property Config is next.
